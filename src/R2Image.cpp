...
BilateralFilter(double sigma, double tolerance)
{
  ...
  for (int y=0; y<height; y++) {
    for (int x=0; x<width; x++) {
	  ...
	  pWeight = 1 - p->GetPixelWeight(&Pixel(x,y));
	  if (1 - pWeight < tolerance) {
...
{
  R2Pixel * ref = new R2Pixel(Pixel(378, 868));
  ...
  std::pair< R2Point*, R2Point* > p1
    (new R2Point(.95*(otherImage->width-1),0),
     markerCoords.at(1));
  std::pair< R2Point*, R2Point* > p2
    (new R2Point(0,.95*(otherImage->height-1)),
     markerCoords.at(2));
  std::pair< R2Point*, R2Point* > p3
    (new R2Point(.95*(otherImage->width-1),.95*(otherImage->height-1)),
     markerCoords.at(3));
