...
static int 
ReadCorrespondences(char *filename, R2Segment *&source_segments,
		    R2Segment *&target_segments, int& nsegments)
{
  // Open file
...
main(int argc, char **argv)
{
  ...
  while (argc > 0) {
    if (!strcmp(*argv, "-brightness")) {
    ...
    else if (!strcmp(*argv, "-highpassfilter")) {
      CheckOption(*argv, argc, 2);
      double sigma = atof(argv[1]);
      argv += 2, argc -= 2;
      image->HighPassFilter(sigma);
    }
      ...
      argv += 3, argc -= 3;
      image->BilateralFilter(sigma, tolerance);
    }
